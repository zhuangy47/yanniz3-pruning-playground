...
from yapt.yapt.analyses import CFG
from yapt.yapt.struct_pruning import FixedPruneRatioPruner, StructuredPruner
from yapt.yapt.struct_pruning.pruning import l2_score_metric
from yapt.yapt.struct_pruning.symch import PruneGroup
...
def prune_resnet18_fix_ratio():
    ...
    CFG(model, (dummy_input,)).prettify_and_draw("resnet18_fix_ratio_prune.png")
...
class OneLayerPruner(StructuredPruner):
    ...
    def select_filters(
        ...
        config = {}
        for group in groups:
            if (self.layer_name in group.nodes):
                config[group] = self.prune_ratio
            else:
                config[group] = 0
        print("Config One Layer Prune Ratio: ")
...
def prune_resnet18_layer():
    ...
    CFG(model, (dummy_input,)).prettify_and_draw("resnet18_one_layer_ratio_prune.png")
...
def prune_resnet18_first_group():
    ...
    CFG(model, (dummy_input,)).prettify_and_draw("resnet18_first_group_prune.png")
